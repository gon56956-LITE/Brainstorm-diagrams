...
- `fmea_table`: simplified FMEA table with item/function, potential failure mode, effects, causes, prevention/detection controls, S/O/D scores, calculated RPN, recommended actions, owner, target date, and status. It is not full AIAG-VDA compliance.
...
- JSON: use `diagram_type` to select `fishbone`, `fault_tree`, `exclusion_tree`, `two_by_two_matrix`, `roadmap_timeline`, or `fmea_table`; fishbone follows `references/input_contract.md`, fault tree follows `brainstorm_diagrams_fault_tree_spec.md`, exclusion tree follows `brainstorm_diagrams_exclusion_tree_spec.md`, two-by-two matrix follows `brainstorm_diagrams_two_by_two_matrix_spec.md`, roadmap timeline follows `brainstorm_diagrams_roadmap_timeline_spec.md`, and FMEA table follows `brainstorm_diagrams_fmea_table_spec.md`.
...
- Markdown FMEA table: include `diagram_type: fmea_table` front matter, optional `language`, a `#` title, `Goal:`, optional project/review lines, and `## Row ...` sections with recognized key-value fields such as `Item / Function:`, `Failure Mode:`, `Effects:`, `Causes:`, `Prevention Controls:`, `Detection Controls:`, `Severity:`, `Occurrence:`, `Detection:`, `Recommended Actions:`, `Owner:`, `Target Completion:`, and `Status:`.
...
3. For a new blank/template diagram, run `scripts/new_fishbone.py <name> --format md`, `scripts/new_fault_tree.py <name> --format md`, `scripts/new_exclusion_tree.py <name> --format md`, `scripts/new_two_by_two_matrix.py <name> --format md --preset action_priority`, `scripts/new_roadmap_timeline.py <name> --format md --preset swimlane_roadmap`, or `scripts/new_fmea_table.py <name> --format md`; use `--format json` when needed.
...
- Implemented diagram types: `fishbone`, `fault_tree`, `exclusion_tree`, `two_by_two_matrix`, `roadmap_timeline`, `fmea_table`.
