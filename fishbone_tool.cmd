...
:menu
cls
echo Fishbone Diagram Tool
echo =====================
echo.
echo 1. Create new fishbone diagram
...
:create
echo.
set /p "name=Diagram name, for example my-analysis: "
if "%name%"=="" (
...
echo.
%PY% "%ROOT%scripts\new_fishbone.py" "%name%" --format "%format%"
echo.
pause
goto menu
...
echo.
%PY% "%ROOT%scripts\render_work.py" "%name%"
echo.
pause
goto menu
...
echo.
%PY% "%ROOT%scripts\export_png.py" "%name%"
echo.
pause
goto menu
...
%PY% "%ROOT%scripts\export_png.py" "%~2"
exit /b %ERRORLEVEL%
