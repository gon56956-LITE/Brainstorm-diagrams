...
:menu
cls
echo FMEA Table Tool
echo ===============
echo.
echo 1. Create new FMEA table
...
echo.
echo Notes:
echo - Diagram names create files under work\fmea-table\.
echo - Use lowercase letters, numbers, hyphen, or underscore only.
echo - Good examples: process-fmea, pilot_yield_fmea
...
:create
echo.
set /p "name=Diagram name, for example process-fmea: "
if "%name%"=="" (
...
echo.
"%PY%" "%ROOT%scripts\new_fmea_table.py" "%name%" --format "%format%"
echo.
pause
goto menu
...
echo.
"%PY%" "%ROOT%scripts\render_fmea_table_work.py" "%name%"
echo.
pause
goto menu
...
echo.
"%PY%" "%ROOT%scripts\export_fmea_table_png.py" "%name%"
echo.
pause
goto menu
...
"%PY%" "%ROOT%scripts\export_fmea_table_png.py" "%~2"
exit /b %ERRORLEVEL%
