...
- Visually inspect `stresscases/full-stress.svg` with `references/visual_review_checklist.md` for overlap, clipped content, spine spacing, topic-block position, and readable category groups.
- Confirm `work/` has no leftover `*tmp*` files.
...
- Add or update `naturalcases/*.source.txt` and `naturalcases/*.expected.md` when changing expected extraction behavior.
...
- Keep `testcases/` for maintained regression inputs and outputs only.
- Keep `naturalcases/` free of generated SVG or PNG files.
- Keep `stresscases/` for optional visual stress review.
