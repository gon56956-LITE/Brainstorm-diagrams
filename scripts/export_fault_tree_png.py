#!/usr/bin/env python3
"""Export a work/fault-tree/ SVG to PNG using the bundled Python runtime."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Export work/fault-tree/<name>.svg to work/fault-tree/<name>.png.")
    parser.add_argument("name", help="Diagram name, such as startup-failure")
...
def validate_work_name(value: str) -> str | None:
    ...
    if not SAFE_NAME_PATTERN.fullmatch(name):
        print(
            ...
            "start with a letter or number; maximum length is 64. Example: startup-failure",
            file=sys.stderr,
        )
        return None
    return name
