#!/usr/bin/env python3
"""Create a new fault-tree workspace input from a template and render its SVG."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a new fault-tree input in work/fault-tree/ and render an initial SVG."
    )
    ...
    parser.add_argument(
        ...
        choices=["md", "json"],
        default="md",
        help="Template format to copy into work/fault-tree/ (default: md)",
    )
    parser.add_argument(
        "--force",
        action="store_true",
...
def main() -> int:
    ...
    template_path = TEMPLATES / f"fault-tree.template.{args.format}"
    ...
    print(f'& "{PYTHON}" scripts\\render_fault_tree_work.py {stem}')
    return 0
