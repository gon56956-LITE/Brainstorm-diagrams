#!/usr/bin/env python3
"""Create a new FMEA table workspace input from a template and render its SVG."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a new FMEA table input in work/fmea-table/ and render an initial SVG."
    )
...
def main() -> int:
    ...
    template_path = TEMPLATES / f"fmea-table.template.{args.format}"
    ...
    print(f'& "{PYTHON}" scripts\\render_fmea_table_work.py {stem}')
    return 0
...
def validate_work_name(value: str) -> str | None:
    ...
    if not SAFE_NAME_PATTERN.fullmatch(name):
        print(
            ...
            "start with a letter or number; maximum length is 64. Example: process-fmea",
            file=sys.stderr,
        )
        return None
    return name
