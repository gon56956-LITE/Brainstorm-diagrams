#!/usr/bin/env python3
"""Create a new roadmap/timeline workspace input from a template and render its SVG."""
...
PRESETS = ("swimlane_roadmap", "milestone_timeline")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a new roadmap/timeline input in work/roadmap-timeline/ and render an initial SVG."
    )
    parser.add_argument("name", help="Diagram name, used as the output file stem")
    parser.add_argument("--format", choices=["md", "json"], default="md", help="Template format to copy (default: md)")
    parser.add_argument(
        "--preset",
        choices=PRESETS,
        default="swimlane_roadmap",
        help="Roadmap/timeline preset template to copy (default: swimlane_roadmap)",
    )
    parser.add_argument("--force", action="store_true", help="Overwrite existing work files for the same name")
...
def main() -> int:
    ...
    template_path = template_for_preset(args.preset, args.format)
    ...
    result = subprocess.run([str(PYTHON), str(GENERATE), str(input_path), str(output_path)], cwd=ROOT, text=True, capture_output=True, check=False)
    if result.stdout:
    ...
    print(f'& "{PYTHON}" scripts\\render_roadmap_timeline_work.py {stem}')
    return 0
...
def validate_work_name(value: str) -> str | None:
    ...
    if not SAFE_NAME_PATTERN.fullmatch(name):
        print(
            ...
            "start with a letter or number; maximum length is 64. Example: product-roadmap",
            file=sys.stderr,
        )
        return None
    return name
...
def template_for_preset(preset: str, format_name: str) -> Path:
    preset_template = TEMPLATES / f"roadmap-timeline.{preset.replace('_', '-')}.template.{format_name}"
    if preset_template.exists():
        return preset_template
    return TEMPLATES / f"roadmap-timeline.template.{format_name}"
