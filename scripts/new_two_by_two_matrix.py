#!/usr/bin/env python3
"""Create a new two-by-two matrix workspace input from a template and render its SVG."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Create a new two-by-two matrix input in work/two-by-two-matrix/ and render an initial SVG."
    )
    ...
    parser.add_argument(
        "--preset",
        choices=PRESETS,
        default="action_priority",
        help="Matrix preset template to copy (default: action_priority)",
    )
...
def main() -> int:
    ...
    print(f'& "{PYTHON}" scripts\\render_two_by_two_matrix_work.py {stem}')
    return 0
...
def validate_work_name(value: str) -> str | None:
    ...
    if not SAFE_NAME_PATTERN.fullmatch(name):
        print(
            ...
            "start with a letter or number; maximum length is 64. Example: action-priority",
            file=sys.stderr,
        )
        return None
    return name
...
def template_for_preset(preset: str, format_name: str) -> Path:
    preset_template = TEMPLATES / f"two-by-two-matrix.{preset.replace('_', '-')}.template.{format_name}"
    if preset_template.exists():
        return preset_template
    return TEMPLATES / f"two-by-two-matrix.template.{format_name}"
