#!/usr/bin/env python3
"""Render an exclusion-tree SVG from a work/exclusion-tree/ input file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render work/exclusion-tree/<name>.md or work/exclusion-tree/<name>.json to work/exclusion-tree/<name>.svg."
    )
    parser.add_argument("name", help="Diagram name or work file stem")
    parser.add_argument(
        "--format",
        choices=["md", "json"],
        help="Input format when both work/exclusion-tree/<name>.md and work/exclusion-tree/<name>.json exist",
    )
...
def resolve_input_path(stem: str, requested_format: str | None) -> Path | None:
    ...
    if not existing:
        print(
            f"Error: no work input found for '{stem}'. Expected work/exclusion-tree/{stem}.md or work/exclusion-tree/{stem}.json.",
            file=sys.stderr,
        )
        return None

    print(
        f"Error: both work/exclusion-tree/{stem}.md and work/exclusion-tree/{stem}.json exist. Use --format md or --format json.",
        file=sys.stderr,
    )
    return None
