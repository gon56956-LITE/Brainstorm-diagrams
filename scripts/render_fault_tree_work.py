#!/usr/bin/env python3
"""Render a fault-tree SVG from a work/fault-tree/ input file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render work/fault-tree/<name>.md or work/fault-tree/<name>.json to work/fault-tree/<name>.svg."
    )
    parser.add_argument("name", help="Diagram name or work file stem")
    parser.add_argument(
        "--format",
        choices=["md", "json"],
        help="Input format when both work/fault-tree/<name>.md and work/fault-tree/<name>.json exist",
    )
...
def resolve_input_path(stem: str, requested_format: str | None) -> Path | None:
    ...
    if not existing:
        print(
            f"Error: no work input found for '{stem}'. Expected work/fault-tree/{stem}.md or work/fault-tree/{stem}.json.",
            file=sys.stderr,
        )
        return None

    print(
        f"Error: both work/fault-tree/{stem}.md and work/fault-tree/{stem}.json exist. Use --format md or --format json.",
        file=sys.stderr,
    )
    return None
...
def validate_work_name(value: str) -> str | None:
    ...
    if not SAFE_NAME_PATTERN.fullmatch(name):
        print(
            ...
            "start with a letter or number; maximum length is 64. Example: startup-fault-tree",
            file=sys.stderr,
        )
        return None
    return name
