#!/usr/bin/env python3
"""Render a roadmap/timeline SVG from a work/roadmap-timeline/ input file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render work/roadmap-timeline/<name>.md or .json to work/roadmap-timeline/<name>.svg."
    )
...
def resolve_input_path(stem: str, requested_format: str | None) -> Path | None:
    ...
    if len(existing) == 1:
        return existing[0]
    if not existing:
        print(f"Error: no work input found for '{stem}'. Expected work/roadmap-timeline/{stem}.md or .json.", file=sys.stderr)
        return None
    print(f"Error: both work/roadmap-timeline/{stem}.md and .json exist. Use --format md or --format json.", file=sys.stderr)
    return None
