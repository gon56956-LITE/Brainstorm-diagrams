#!/usr/bin/env python3
"""Render a two-by-two matrix SVG from a work/two-by-two-matrix/ input file."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render work/two-by-two-matrix/<name>.md or .json to work/two-by-two-matrix/<name>.svg."
    )
    parser.add_argument("name", help="Diagram name or work file stem")
    parser.add_argument("--format", choices=["md", "json"], help="Input format when both .md and .json inputs exist")
...
def resolve_input_path(stem: str, requested_format: str | None) -> Path | None:
    ...
    if len(existing) == 1:
        return existing[0]
    if not existing:
        print(f"Error: no work input found for '{stem}'. Expected work/two-by-two-matrix/{stem}.md or .json.", file=sys.stderr)
        return None
    print(f"Error: both work/two-by-two-matrix/{stem}.md and .json exist. Use --format md or --format json.", file=sys.stderr)
    return None
